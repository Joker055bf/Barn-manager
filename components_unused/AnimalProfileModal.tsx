  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-md w-full shadow-2xl">
        <h2 className="text-xl font-bold mb-4">╪د┘┘à┘┘ ╪د┘╪┤╪«╪╡┘è ┘┘╪ص┘è┘ê╪د┘</h2>
        <p className="text-gray-500 mb-6">┘ç╪░╪د ╪د┘┘à┘â┘ê┘ ╪║┘è╪▒ ┘à╪│╪ز╪«╪»┘à ╪ص╪د┘┘è╪د┘ï ┘┘è ╪د┘┘╪│╪«╪ر ╪د┘╪ص╪د┘┘è╪ر.</p>
        <button onClick={onClose} className="w-full bg-[#795548] text-white py-3 rounded-2xl">╪ح╪║┘╪د┘é</button>
      </div>
    </div>
  );
};
...
export const AnimalProfileModal: React.FC<AnimalProfileModalProps> = ({
  ...
  const getPenName = (penId: string) => {
    ...
    return pen ? pen.name : '╪║┘è╪▒ ┘à╪ص╪»╪»';
  };

  const getAgeName = (birthDate: string) => {
    if (!birthDate) return '╪║┘è╪▒ ┘à╪╣╪▒┘ê┘';
    const months = Math.floor((new Date().getTime() - new Date(birthDate).getTime()) / (1000 * 60 * 60 * 24 * 30.44));
    if (months < 6) return '╪ذ┘ç┘à╪ر';
    if (months < 12) return '╪ش╪░╪╣';
    if (months < 24) return '╪س┘┘è';
    if (months < 36) return '╪▒╪ذ╪د╪╣';
    if (months < 48) return '╪│╪»┘è╪│';
    return '╪ش╪د┘à╪╣';
  };

  const getAgeString = (birthDate: string) => {
    if (!birthDate) return '╪║┘è╪▒ ┘à╪ص╪»╪»';
    const months = Math.floor((new Date().getTime() - new Date(birthDate).getTime()) / (1000 * 60 * 60 * 24 * 30.44));
    if (months < 1) return '╪ث┘é┘ ┘à┘ ╪┤┘ç╪▒';
    if (months < 12) return `${months} ╪┤┘ç╪▒`;
    const years = Math.floor(months / 12);
    const remMonths = months % 12;
    return `${years} ╪│┘╪ر${remMonths > 0 ? ` ┘ê ${remMonths} ╪┤┘ç╪▒` : ''}`;
  };

  // Movements from log
  const movements = useMemo(() => {
    return activityLog.filter(log =>
      (log.action === '┘┘é┘ ╪ص┘è┘ê╪د┘' || log.action === '╪ز╪ص╪▒┘è┘â ╪ص┘è┘ê╪د┘' || log.action === '┘┘é┘ ╪ش┘à╪د╪╣┘è') &&
...
export const AnimalProfileModal: React.FC<AnimalProfileModalProps> = ({
  ...
  return (
    ...
    ar: {
      profileTitle: '╪د┘┘à┘┘ ╪د┘╪┤╪د┘à┘ ┘┘╪ص┘è┘ê╪د┘',
      basicTab: '┘à┘╪«╪╡ ╪د┘╪ص╪د┘╪ر',
      timelineTab: '╪د┘┘à╪│╪د╪▒ ╪د┘╪▓┘à┘┘è',
      productionTab: '╪│╪ش┘ ╪د┘╪ح┘╪ز╪د╪ش',
      serialNumber: '╪▒┘é┘à ╪د┘╪│╪د╪▒┘è╪ر',
      breed: '╪د┘┘┘ê╪╣',
      gender: '╪د┘╪ش┘╪│',
      age: '╪د┘╪╣┘à╪▒ ╪د┘╪»┘é┘è┘é',
      tagColor: '┘┘ê┘ ╪د┘┘é┘è╪»',
      nickname: '╪د┘┘┘é╪ذ',
      matingDate: '╪ز╪د╪▒┘è╪« ╪د┘╪ز┘┘é┘è╪ص',
      sireId: '╪▒┘é┘à ╪د┘┘╪ص┘',
      expectedBirthDate: '╪ز╪د╪▒┘è╪« ╪د┘┘ê┘╪د╪»╪ر ╪د┘┘à╪ز┘ê┘é╪╣',
      daysLeft: '┘è┘ê┘à ┘à╪ز╪ذ┘é┘è',
      daysAgo: '┘è┘ê┘à ┘à╪╢┘ë',
          >
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 p-4 rounded-2xl dark:bg-slate-800">
                  <span className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">╪د┘┘┘ê╪╣</span>
                  <span className="text-sm font-bold text-gray-900 dark:text-white">{sheep.type}</span>
                </div>
                <div className="bg-gray-50 p-4 rounded-2xl dark:bg-slate-800">
                  <span className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">╪د┘╪ش┘╪│</span>
                  <span className="text-sm font-bold text-gray-900 dark:text-white">{sheep.gender === 'male' ? '╪░┘â╪▒' : '╪ث┘╪س┘ë'}</span>
                </div>
                <div className="bg-gray-50 p-4 rounded-2xl dark:bg-slate-800">
                  <span className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">╪ز╪د╪▒┘è╪« ╪د┘┘à┘è┘╪د╪»</span>
                  <span className="text-sm font-bold text-gray-900 dark:text-white">{sheep.birthDate || '╪║┘è╪▒ ┘à╪│╪ش┘'}</span>
                </div>
                <div className="bg-gray-50 p-4 rounded-2xl dark:bg-slate-800">
                  <span className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">╪د┘╪╣┘à╪▒ ╪د┘╪»┘é┘è┘é</span>
                  <span className="text-sm font-bold text-gray-900 dark:text-white">{getAgeString(sheep.birthDate)}</span>
                </div>
                <div className="bg-gray-50 p-4 rounded-2xl dark:bg-slate-800">
                  <span className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">╪د╪│┘à ╪د┘╪╣┘à╪▒</span>
              ...
              <div className="space-y-4">
                <h3 className="font-black text-sm text-gray-800 flex items-center gap-2 dark:text-white">
                  ...
                  ╪د┘╪ص╪د┘╪ر ╪د┘╪╡╪ص┘è╪ر ┘ê╪د┘╪ح┘╪ش╪د╪ذ┘è╪ر
                </h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className={`p-4 rounded-2xl border flex items-center gap-3 ${sheep.healthStatus === 'sick' ? 'bg-red-50 border-red-100 text-red-700 dark:bg-red-900/20 dark:border-red-800/30 dark:text-red-400' : 'bg-emerald-50 border-emerald-100 text-emerald-700 dark:bg-emerald-900/20 dark:border-emerald-800/30 dark:text-emerald-400'}`}>
                    ...
                    <div>
                      <span className="block text-[10px] font-black uppercase tracking-widest opacity-80 mb-0.5">╪د┘╪ص╪د┘╪ر ╪د┘╪╡╪ص┘è╪ر</span>
                      <span className="text-sm font-bold">{sheep.healthStatus === 'sick' ? '┘à╪▒┘è╪╢' : '╪│┘┘è┘à'}</span>
                    </div>
                  </div>
                  ...
                  {sheep.gender === 'female' && (
                    <div className="p-4 rounded-2xl border bg-purple-50 border-purple-100 text-purple-700 flex items-center gap-3 dark:bg-purple-900/20 dark:border-purple-800/30 dark:text-purple-400">
                      <Baby size={24} />
                      <div>
                        <span className="block text-[10px] font-black uppercase tracking-widest opacity-80 mb-0.5">╪ص╪د┘╪ر ╪د┘╪ح┘╪ش╪د╪ذ</span>
                        <span className="text-sm font-bold">
                          {sheep.reproductionStatus === 'pregnant' ? '┘à╪╢╪▒╪╣ (╪ص╪د┘à┘)' : sheep.reproductionStatus === 'mother' ? '╪ث┘à ┘à╪▒╪╢╪╣╪ر' : '╪║┘è╪▒ ┘à╪╢╪▒╪╣'}
                        </span>
                      </div>
                    </div>
                  )}
                      ...
                      <div className="bg-gray-50 p-4 rounded-2xl dark:bg-slate-800">
                        ...
                        <div className="flex items-center gap-3 text-sm font-bold text-gray-800 dark:text-gray-200">
                          <div className="flex-1 bg-white p-3 rounded-xl border border-gray-100 text-center dark:bg-slate-900 dark:border-slate-700">
                            <span className="block text-[10px] text-gray-400 mb-1">┘à┘</span>
                            {move.from}
                          </div>
    ...
    if (months < 0) {
      ...
      months += 12;
                          ...
                          <div className="text-xs font-bold text-gray-400 flex items-center gap-2">
                            <span>┘ê┘╪د╪»╪ر: {child.birthDate}</span>
                            </span>
                          </div>
                          <div className="text-xs font-bold text-gray-400 flex items-center gap-2">
                            <span>┘ê┘╪د╪»╪ر: {child.birthDate}</span>
                      ...
                      <div className="text-left">
                        <span className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">╪د┘┘à┘â╪د┘ ╪د┘╪ص╪د┘┘è</span>
                ...
                <div className="py-12 text-center text-gray-400">
                  ...
                  <p className="text-sm font-bold">┘┘à ┘è╪ز┘à ╪ز╪│╪ش┘è┘ ╪ث┘è ┘à┘ê╪د┘┘è╪» ┘┘ç╪░┘ç ╪د┘╪ث┘╪س┘ë</p>
                </div>
              )}
            </div>
          )}
          ...
          if (onActivityLogged) {
            onActivityLogged(
              language === 'ar' ? '╪ح╪ش╪▒╪د╪ة ┘╪╖╪د┘à ┘è╪»┘ê┘è' : 'Manual Weaning',
              language === 'ar' 
                ? `╪ز┘à ╪ح╪ز┘à╪د┘à ┘╪╖╪د┘à ╪د┘╪┤╪د╪ر #${sheep.serialNumber} ┘è╪»┘ê┘è╪د┘ï ┘ê╪ح╪▒╪ش╪د╪╣┘ç╪د ┘╪ص╪د┘╪ر ╪║┘è╪▒ ┘à╪╢╪▒╪╣` 
                : `Ewe #${sheep.serialNumber} has been manually weaned and set back to empty`
            );
          }
          onShowAlert('success', language === 'ar' ? '╪ز┘à ╪د┘┘╪╖╪د┘à' : 'Weaned', language === 'ar' ? '╪ز┘à ╪ح┘┘ç╪د╪ة ┘╪ز╪▒╪ر ╪د┘╪▒╪╢╪د╪╣╪ر ╪ذ┘╪ش╪د╪ص.' : 'Lactation finished successfully.');
        } catch (e) {
          console.error(e);
          onShowAlert('error', '╪«╪╖╪ث', '┘╪┤┘ ┘┘è ╪ز╪ص╪»┘è╪س ╪د┘╪ذ┘è╪د┘╪د╪ز.');
        }
      }
    );
  };
  ...
  const handleSaveMating = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canEdit) {
      onShowAlert('error', '╪«╪╖╪ث', loc.permissionError);
      return;
    }
    if (!matingSireId.trim()) {
      onShowAlert('error', '╪ص┘é┘ ┘à╪╖┘┘ê╪ذ', language === 'ar' ? '┘è╪▒╪ش┘ë ╪ح╪»╪«╪د┘ ╪ث┘ê ╪د╪«╪ز┘è╪د╪▒ ╪▒┘é┘à ╪د┘┘╪ص┘' : 'Please input sire ID');
      return;
    }
    ...
    try {
      ...
      console.error(e);
      onShowAlert('error', '╪«╪╖╪ث', '┘╪┤┘ ┘┘è ╪ص┘╪╕ ╪ذ┘è╪د┘╪د╪ز ╪د┘┘ê┘╪د╪»╪ر.');
    }
  };
  ...
  const getTimelineEvents = () => {
    ...
    if (sheep.reproductionHistory) {
      sheep.reproductionHistory.forEach((h: any) => {
        ...
        if (h.type === 'mating') {
          title = language === 'ar' ? '╪ز┘┘é┘è╪ص ┘ê╪ح╪«╪╡╪د╪ذ' : 'Mating';
          color = 'bg-blue-500 text-blue-500';
        } else if (h.type === 'birth') {
          title = language === 'ar' ? '┘ê┘╪د╪»╪ر ┘ê╪ح┘╪ز╪د╪ش' : 'Birth Delivery';
          color = 'bg-emerald-500 text-emerald-500';
        } else if (h.type === 'abortion') {
          title = language === 'ar' ? '╪ص╪د┘╪ر ╪ح╪ش┘ç╪د╪╢' : 'Abortion / Emergency';
          color = 'bg-red-500 text-red-500';
        } else if (h.type === 'weaning') {
          title = language === 'ar' ? '╪ح╪ز┘à╪د┘à ╪د┘┘╪╖╪د┘à' : 'Weaning Completed';
    ...
    activityLog.forEach((log: any) => {
      ...
      const isMove = log.action?.includes('┘┘é┘') || log.action?.includes('┘┘é┘ ╪ش┘à╪د╪╣┘è') || log.action?.includes('Move');
      if (isRelated && isMove) {
        events.push({
          id: log.id,
          date: log.timestamp?.split('T')[0] || '',
          type: 'transfer',
          title: language === 'ar' ? '┘┘é┘ ╪د┘╪ص╪╕┘è╪▒╪ر' : 'Barn Transfer',
          detail: log.detail,
  ...
  const getProductionStats = () => {
    ...
    births.forEach((b: any) => {
      ...
      } else if (b.details.includes('╪ز┘ê╪ث┘à') || b.details.includes('twin')) {
        totalKids += 2;
      } else if (b.details.includes('╪س┘╪د╪س╪ر') || b.details.includes('triplet')) {
        totalKids += 3;
      } else {
        totalKids += 1;
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="bg-[#fcfbf4] w-full max-w-2xl h-[90vh] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col relative dark:bg-slate-950 border border-white/20">
        ...
        <div className="flex bg-gray-50 border-b border-gray-100 dark:bg-slate-900/50 dark:border-slate-800/80 p-2 gap-2">
          ...
          >
            <Award size={16} />
            {sheep.gender === 'female' ? loc.productionTab : (language === 'ar' ? '╪│╪ش┘ ╪د┘┘╪│┘ ┘ê╪د┘┘╪ز╪د╪ش' : 'Offspring & Lineage')}
          </button>
        </div>
                  ...
                  <div className={`p-5 rounded-3xl border flex flex-col md:flex-row justify-between items-center gap-4 ${
                    ...
                    <div className="flex items-center gap-3">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                        ...
                        <div className="text-left">
                          <span className="text-[10px] font-black text-gray-400 block uppercase tracking-widest">{language === 'ar' ? '┘╪ز╪▒╪ر ╪د┘╪▒╪╢╪د╪╣╪ر' : 'Lactation Duration'}</span>
                          <span className="text-xs font-black text-blue-600 mt-1 block">
                            {weaning.daysPassed} {language === 'ar' ? '┘è┘ê┘à ┘à╪╢┘ë' : 'days passed'}
                          </span>
                        </div>
                      </div>
                      ...
                      {sheep.lastBirthDate && (
                        <div className="flex justify-between items-center text-xs border-t border-gray-100 dark:border-slate-800/80 pt-3 text-gray-500">
                          <span>{language === 'ar' ? '╪ز╪د╪▒┘è╪« ╪د┘┘ê┘╪د╪»╪ر:' : 'Birth Date:'} <strong>{sheep.lastBirthDate}</strong></span>
                        </div>
                      )}
                    </div>
                  )}
                    ...
                    <form onSubmit={handleSaveBirth} className="bg-gray-50 dark:bg-slate-850 p-5 rounded-3xl border border-gray-100 dark:border-slate-800/80 space-y-4 mt-4 animate-slide-up max-h-[50vh] overflow-y-auto custom-scrollbar">
                      <div className="flex justify-between items-center">
                        <h5 className="font-black text-sm text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                          <Baby size={16} />
                          {language === 'ar' ? '╪ز╪│╪ش┘è┘ ┘à┘ê╪د┘┘è╪» ╪ش╪»╪»' : 'Register Delivery / Birth'}
                        </h5>
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="flex gap-2 w-full mt-6 border-t border-gray-100 dark:border-slate-800/80 pt-4">
                ...
                {currentUser && (isOwner || currentUser.permissions?.canAddMedical) && onVaccinateSheep && (
                  ...
                  >
                    <Syringe size={18} />
                    {language === 'ar' ? '╪ز╪ص╪╡┘è┘ / ╪╣┘╪د╪ش' : 'Vaccinate'}
                  </button>
                )}
                ...
                {currentUser && (isOwner || currentUser.permissions?.canMoveAnimals) && onMoveSheep && (
                  ...
                  >
                    ...
                    {language === 'ar' ? '┘┘é┘ ╪د┘┘é╪│┘à' : 'Move Section'}
                  </button>
                )}
                ...
                {currentUser && (isOwner || currentUser.permissions?.canEditAnimals) && onEditSheep && (
                  ...
                  >
                    <Edit size={18} />
                    {language === 'ar' ? '╪ز╪╣╪»┘è┘ ╪د┘╪ذ┘è╪د┘╪د╪ز' : 'Edit Info'}
                  </button>
                )}
              </div>
            </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div className="bg-blue-50/30 border border-blue-100/50 p-4 rounded-3xl text-center dark:bg-blue-950/10 dark:border-blue-900/20">
                      <span className="text-[9px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest block">{language === 'ar' ? '╪ح╪ش┘à╪د┘┘è ╪د┘┘╪ز╪د╪ش' : 'Total Kids'}</span>
                  ...
                  <div className="space-y-4">
                    <h5 className="font-black text-sm text-[#3E2723] dark:text-white uppercase tracking-widest">{language === 'ar' ? '╪│╪ش┘ ╪د┘┘ê┘╪د╪»╪د╪ز ╪د┘╪│╪د╪ذ┘é╪ر' : 'Past Deliveries History'}</h5>
                ...
                (() => {
                  ...
                  const sireAverageL
                                  ...
                                  >
                                    ...
                                    >
                                      #{foundMother ? foundMother.serialNumber : del.motherId || '╪║┘è╪▒ ┘à╪╣╪▒┘ê┘'}
                                    </button>
                                  </div>

                                  <div className="flex flex-wrap gap-1.5 items-center justify-end">
                                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block w-full text-right md:w-auto md:text-left mr-2">
                                      {language === 'ar' ? '╪د┘┘à┘ê╪د┘┘è╪»:' : 'Offsprings:'}
                                    </span>
                                    {del.kids.map((kid: Sheep) => (
                                      ...
                                      >
                                        #{kid.serialNumber} ({kid.gender === 'male' ? (language === 'ar' ? '╪░┘â╪▒' : 'M') : (language === 'ar' ? '╪ث┘╪س┘ë' : 'F')})
                                      </button>
                                    ))}
                                  </div>
                                </div>
                                ...
                                ? (birth.offspringSerials.length === 1 ? loc.single : birth.offspringSerials.length === 2 ? loc.twin : `${birth.offspringSerials.length} ${language === 'ar' ? '╪ز┘ê╪د╪خ┘à' : 'Kids'}`)
