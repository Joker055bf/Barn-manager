...
export const VaccinationGuide: React.FC<VaccinationGuideProps> = ({ sheepList = [] }) => {
  const [activeTab, setActiveTab] = useState<'log' | 'guide'>('log');
  const [searchTerm, setSearchTerm] = useState('');

  const vaccines = [
    { name: 'التسمم المعوي (الدموي)', age: 'من شهرين', frequency: 'كل 6 أشهر', notes: 'أهم تطعيم، يسبب الموت المفاجئ' },
    { name: 'الجدري', age: '3-4 أشهر', frequency: 'سنوي', notes: 'يسبب حبوب وتقرحات' },
    { name: 'الطاعون (PPR)', age: '3-4 أشهر', frequency: 'مرة بالعمر (مناعة طويلة)', notes: 'مرض فيروسي خطير' },
    { name: 'القلاعية (FMD)', age: '3 أشهر', frequency: 'كل 6 أشهر', notes: 'يسبب عرج وتقرحات بالفم' },
    { name: 'البروسيلا (المالطية)', age: '3-6 أشهر (إناث فقط)', frequency: 'مرة واحدة', notes: 'يسبب الإجهاض، ينتقل للإنسان' },
    { name: 'اللاهوائيات (7 أمراض)', age: 'أسبوعين', frequency: 'سنوي', notes: 'شامل لعدة أمراض بكتيرية' },
  ];
      ...
      {activeTab === 'guide' && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-scale-in">
          <div className="bg-purple-600 p-6 text-white">
            ...
            <p className="text-purple-100 mt-1 text-sm">جدول استرشادي لأهم التحصينات الدورية للأغنام</p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-scale-in">
          <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-50">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1">قائمة بجميع التطعيمات والعلاجات المسجلة للأغنام</p>
            </div>
          ...
          <div className="p-0">
            {medicalHistory.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-xs text-gray-300 mt-1">أضف تطعيم أو علاج من صفحة الأغنام</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                         ...
                         <tr key={record.recordId} className="hover:bg-gray-50/50 transition">
                           ...
                           <td className="p-4">
                              <span className={`px-2 py-1 rounded-md text-xs font-bold ${
                                record.type === 'vaccine' ? 'bg-purple-100 text-purple-700' :
