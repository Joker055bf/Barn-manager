...
interface FeedManagerProps {
  ...
  penId: string; // Required to filter and tag items
  animalType?: string; // Optional animal type for conditional logic
}

export const FeedManager: React.FC<FeedManagerProps> = ({ items, onUpdate, penId, animalType }) => {
  ...
  // Filter items for current pen AND selected category
  const penItems = items.filter(i => i.penId === penId && i.category === category);
  ...
  const handleAddOrUpdate = (e: React.FormEvent) => {
    ...
    const existingIndex = items.findIndex(i => i.name.trim() === newName.trim() && i.penId === penId);
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">مخزون هذه الحظيرة فقط</p>
          </div>
        </div>
        ...
        <form onSubmit={handleAddOrUpdate} className="flex flex-col gap-4">
          ...
          {/* Show visual indicator for Birds that only Grain is allowed */}
          {isBirds && (
            <div className="bg-orange-50 text-orange-800 p-3 rounded-xl flex items-center gap-2 text-sm font-bold border border-orange-100">
              <Wheat size={18} />
              <span>مخصص للحبوب فقط (دواجن/حمام)</span>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {penItems.length === 0 && (
          <div className="col-span-full flex flex-col items-center justify-center py-20 text-gray-400 bg-white rounded-3xl border-2 border-dashed border-gray-200">
            ...
            <p className="text-lg font-medium">المخزون فارغ من {category === 'grain' ? 'الحبوب' : 'الأعلاف'}</p>
            <p className="text-sm">أضف {category === 'grain' ? 'الحبوب' : 'الأعلاف'} من النموذج بالأعلى</p>
          </div>
        )}
